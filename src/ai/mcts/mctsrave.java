...
public class mctsrave implements AI {
    ...
    public mctsrave(long budget, IStateEvaluator defaultPolicy) {
        this.budget = budget;
    ...
    private MctsEdge best(MctsNode node, boolean urgent) {
        ...
        int actionCounter = 0;
        int totalAMAFusedValues = 0;
        Action action = null;
        double bestVal = -100000;
        MctsEdge bestEdge = null;
        for (final MctsEdge edge : node.out) {
            actionCounter +=edge.visits;
            Totalindicator += AMAF(edge)[0];
            totalAMAFusedValues += AMAF(edge)[1];
            final double val = rave(edge, node, urgent, totalAMAFusedValues, Totalindicator, actionCounter);
            if (val > bestVal) {
                bestVal = val;
                bestEdge = edge;
            }
     ...
     * @return
     */
    private double rave(MctsEdge edge, MctsNode node, boolean urgent, int totalAMAFusedValues, int AmafSimulation, int actionCounter) {
        double N = edge.visits;
        double Nhat = AmafSimulation;
        int MoveSelected = 0;

        for(int i =0; i < edge.from.in.size();i++){
            if(edge.action == edge.from.in.get(i).action){
                MoveSelected += 1;
            }
        }
        double b = c;
        double B = Nhat/(N + Nhat + 4*(N)*(Nhat)*b);
        double Q = 1/N*(MoveSelected);
        double Qhat = (1/Nhat)*(totalAMAFusedValues);
        return (1 - B)*Q + B*Qhat;
    }

    private int[] AMAF(MctsEdge edge) {
