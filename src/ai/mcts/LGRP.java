...
public class LGRP implements AI {
    ...
    public ArrayList<MctsEdge> edgelist;

    public LGRP(long budget, IStateEvaluator defaultPolicy) {
        this.budget = budget;
        this.defaultPolicy = defaultPolicy;
        pruner = new ActionPruner();
        comparator = new ComplexActionComparator();
        move = new ArrayList<Action>();
        edgelist = new ArrayList<MctsEdge>();
        ends = 0;
        c = 1 / Math.sqrt(2);
        cut = false;
        collapse = false;
    ...
    public Action act(GameState state, long ms) {
        ...
        while (time > 0) {
            ...
            delta1 = defaultPolicy.eval(clone, state.p1Turn);

            // BACKPROPAGATION
            backupNegaMax(traversal, delta1, state.p1Turn);
    ...
    private MctsEdge best(MctsNode node, boolean urgent) {
        ...
        for (final MctsEdge edge : node.out) {
            ...
            if (val > bestVal) {
                ...
                edge.value = bestVal;
                bestEdge = edge;
                System.out.println("edgelist before adding");
                System.out.println(edgelist);
                edgelist.add(bestEdge);
                System.out.println("edgelist after adding");
                System.out.println(edgelist);
                bestEdge = sort(edgelist, bestEdge);
                results.add(bestEdge.value);
            }else{
                System.out.println("skipping adding to edgelist because bestval is null");
            }
        }
        return bestEdge;
    }

    private MctsEdge sort(ArrayList<MctsEdge> edgelist, MctsEdge bestEdge) {
        double value = edgelist.get(0).value;
        int index = 0;
        for(int i = 0; i < edgelist.size()-1; i++){
            System.out.println("printing out value of i ");
            System.out.println(i);
            System.out.println(edgelist.get(i).value);
            if(value <= edgelist.get(i).value){
                value = edgelist.get(i).value;
                index = i;
            }
        }
        System.out.println("printing out value of index ");
        System.out.println(index);
        System.out.println(edgelist);
        if(edgelist.get(index).action != bestEdge.action){
            if(bestEdge.to.possible.contains(edgelist.get(index).action)){
                if(edgelist.get(index).to.out.contains(bestEdge)){
                    return edgelist.get(index);
                }
                else {
                    bestEdge.action = edgelist.get(index).action;
                    return bestEdge;
                }
            }else{
                return bestEdge;
